...
public class ControlSettingVIewCase2 : BaseMono<ControlSettingVIewCase2>
{
    ...
    void OnOKButtonClick()
    {
        ...
        //컨베이어
        ConSpeed?.Invoke(conSpeedSli.value);
        //반출공정
        AMachineDuration?.Invoke(amSpeedSli.value);
        AMachineTime?.Invoke(amIntervalSli.value);
        //투입기공정
        BMachineDuratio?.Invoke(bmSpeedSli.value);
        //가공공정
        CMachineDuration?.Invoke(cmSpeedSli.value);
        CMachineTime?.Invoke(cmManufacturingSli.value);
        //로봇팔공정
    ...
    void LoadInitialSettings()
    {
        ...
        //총갯수
        strobj = string.Format("{0}", GameObject.Find("CASE").GetComponent<Manage>().TotalAmount);
        ApplySetting("loadCapacity", strobj);
        
        

        //폼목
        strobj = string.Format("{0}", GameObject.Find("M02").GetComponent<Case2M02>().GetContent()); 
        ApplySetting("mainContent", strobj);

        //용량
        strobj = string.Format("{0}", (GameObject.Find("M02").GetComponent<Case2M02>().M02Time/3)-1);
        ApplySetting("mainCapacity", strobj);

        //컨베이어
        strobj = string.Format("{0}", GameObject.Find("Conveyor").GetComponent<Case2MJConveyor>().ConvSpeedRatio);
        ApplySetting("conSpeedTxt", strobj);
         
        //AMachine
        strobj = string.Format("{0}", GameObject.Find("M01").GetComponent<Case2M01>().M1SpeedRatio);
        ApplySetting("amSpeedTxt", strobj);

        strobj = string.Format("{0}", GameObject.Find("M01").GetComponent<Case2M01>().M01Time);
        ApplySetting("amIntervalTxt", strobj);

        //BMachine
        strobj = string.Format("{0}", GameObject.Find("M02").GetComponent<Case2M02>().M2SpeedRatio);
        ApplySetting("bmSpeedTxt", strobj);

        //CMachine
        strobj = string.Format("{0}", GameObject.Find("M03").GetComponent<Case2M03>().M3SpeedRatio);
        ApplySetting("cmSpeedTxt", strobj);

        strobj = string.Format("{0}", GameObject.Find("M03").GetComponent<Case2M03>().M03Time);
    ...
    void ApplySetting(string id, string value) 
    {
        switch (id)
        {
            ...
            case "rb2stSpeedTxt":
                ...
                break;
            case "mainContent":
                mainContent.value = ((int)GameObject.Find("M02").GetComponent<Case2M02>().GetContent());
                break;
            case "mainCapacity":
