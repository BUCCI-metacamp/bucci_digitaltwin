...
public class CaseMqtt : MonoBehaviour
{
    MQTTClient client;
    ...
    {
        //GetMqttSetting();
        GameObject.Find("MQTT").GetComponent<CaseControllMessage>().onControllMessage += PublishMessage;

        //Connect("http://158.247.241.162/", "1883", "simulation/optimal/data");
    }
    ...
    private void OnConnected(MQTTClient client)
    {
        ...
        if (caseType == 1)
        {
            var edukit = FindObjectOfType<Factory.Manage>();
        ...
        {
            var edukit = FindObjectOfType<Factory.ManageCase2>();
            for (int i = 0; i < subscriptionTopics.Length; i++)
            {
                SubscriptionTopic(subscriptionTopics[i], edukit.ReceiveData);
                client.AddTopicAlias(subscriptionTopics[i]);
    ...
    Dictionary<string, Action<List<AIReceiveData>>> topicSubscriptionTable = new();
    private void OnMessage(MQTTClient client, SubscriptionTopic topic, string topicName, ApplicationMessage message)
    {
        ...
        List<AIReceiveData> data = JsonConvert.DeserializeObject<List<AIReceiveData>>(payload);
        Debug.Log("asdfsdf" + data.Count);
        topicSubscriptionTable[topicName]?.Invoke(data);
    }

    public void SubscriptionTopic(string topic, Action<List<AIReceiveData>> callback)
    {
        Debug.Log("asdfsdfsdf");
    ...
    public void PublishMessage(string message)
    {
        ...
        string controllTopic = "simulation/request";
        byte[] payload = Encoding.UTF8.GetBytes(message);
        Debug.Log("payload::::" + payload);
        Debug.Log("controllTopic::::" + controllTopic);
